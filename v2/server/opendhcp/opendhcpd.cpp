...
const char sVersion[] = "Open DHCP Server Version 1.60 Linux Build 1037";
...
void *init(void *lparam)
{
	...
	{
		if (findServer(newNetwork.staticServers, MAX_SERVERS, cfig.zoneServers[0]) && findServer(newNetwork.staticServers, MAX_SERVERS, cfig.zoneServers[1]))
		{
			sprintf(logBuff, "Section [REPLICATION_SERVERS] Primary & Secondary should be Different Boxes");
			logDHCPMess(logBuff, 1);
		}
		else if (findServer(newNetwork.staticServers, MAX_SERVERS, cfig.zoneServers[0]))
			cfig.replication = 1;
		else if (findServer(newNetwork.staticServers, MAX_SERVERS, cfig.zoneServers[1]))
			cfig.replication = 2;
		else
		{
	...
	do
	{
				...
				if(i < MAX_SERVERS)
				{
					...
					if (isIP(raw) && addr)
					{
						for (MYBYTE m = 0; ; m++)
						{
							...
							{
								if (findServer(newNetwork.allServers, MAX_SERVERS, addr))
								{
		...
		if (!cfig.ifspecified)
		{
			for (MYBYTE n = 0; n < MAX_SERVERS && newNetwork.staticServers[n]; n++)
			{
				...
				newNetwork.listenMasks[n] = newNetwork.staticMasks[n];
			}
		}
	...
	if (ioctl(cfig.fixedSocket, SIOCGIFCONF, &Ifc) >= 0)
	{
		...
		for (MYBYTE i = 0 ; i < numInterfaces; i++)
		{
			...
			if (addr && mask && !(flags & IFF_POINTOPOINT) && !(flags & IFF_LOOPBACK) && !(flags & IFF_DYNAMIC))
			{
				...
				{
					for (MYBYTE m = 0; m < MAX_SERVERS; m++)
					{
						if (network->staticServers[m] == addr)
							break;
						else if (!network->staticServers[m])
						{
							network->staticServers[m] = addr;
							network->staticMasks[m] = mask;
							break;
						}
					}
				}
