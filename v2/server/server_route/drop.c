	               ...
	               int (*okfn)(struct sk_buff *) )
{
	...
	unsigned char DMAC_route[ETH_ALEN]={0x0c,0x82,0x68,0x66,0x30,0x25};         	//route
	...
	state_wlan=eth3->state;
	state_eth=eth4->state;
	
	printk(KERN_INFO "eth3 state: %d\n", state_wlan);
	printk(KERN_INFO "eth4 name: %d\n", state_eth);
	...
	if(ipheader->protocol==IPPROTO_TCP)
	{
		...
		if(destination_ip==test_ip)
		{
			...
			else if(tcpheader->syn==0&&state_eth==3)
			{
				skb_recv->dev=eth4;
				skb_recv->mac_header=skb_push(skb,14);
				ethdr=(struct ethhdr*)skb_recv->mac_header;
				memcpy(ethdr->h_dest,DMAC_route,ETH_ALEN);
